...
const formatAttributes = (unit: EvidenceUnit) => {
  const attributes = unit.attributes || {};
  const parts: string[] = [];

  if (attributes.temporality) parts.push(`Timing: ${attributes.temporality}`);
  if (attributes.frequency) parts.push(`Frequency: ${attributes.frequency}`);
  if (attributes.severity) parts.push(`Severity: ${attributes.severity}`);
  if (attributes.attribution) parts.push(`Attribution: ${attributes.attribution}`);
...
const EntryDetailV2Page = () => {
  ...
  const evidenceGroups = useMemo(() => {
    ...
    const groups = new Map<string, { label: string; patientLabel: string; prompt?: string; units: EvidenceUnit[] }>();
    (data?.evidenceUnits || []).forEach((unit) => {
      const mappingEntry = labelMappings[unit.label];
      const patientLabel = mappingEntry?.patient_label || humanizeLabel(unit.label);
      const prompt = mappingEntry?.reflection_prompt;
      if (!groups.has(unit.label)) {
        groups.set(unit.label, { label: unit.label, patientLabel, prompt, units: [] });
      }
      groups.get(unit.label)?.units.push(unit);
    });
    return Array.from(groups.values());
  }, [data?.evidenceUnits, mapping.evidence_label_mappings]);

  const unclearAreas = useMemo(
    () => buildDerivedPrompts(data?.evidenceUnits || [], mapping.derived_prompts?.unclear_areas),
    [data?.evidenceUnits, mapping.derived_prompts?.unclear_areas],
  );

  const questionsToExplore = useMemo(
    () => buildDerivedPrompts(data?.evidenceUnits || [], mapping.derived_prompts?.questions_to_explore),
    [data?.evidenceUnits, mapping.derived_prompts?.questions_to_explore],
  );
  ...
  return (
    <div className="space-y-6">
      <Card className="border p-8 text-slate-900">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-brandLight">Patient view (v2)</p>
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="secondary" onClick={() => navigate(`/patient/entry/${data.id}`)}>
              Legacy detail
            </Button>
            <Button onClick={() => navigate("/patient/journal")}>Back to journal</Button>
          </div>
        </div>
        <div className="mt-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 border p-6">
          ...
          {evidenceGroups.length ? (
              ...
              {evidenceGroups.map((group) => (
                <div key={group.label} className="rounded-2xl border border-slate-100 bg-white p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <Badge className="bg-slate-100 text-slate-600">{group.label}</Badge>
                  </div>
                  {group.prompt ? (
                  ...
                  <div className="mt-3 space-y-2 text-sm text-slate-600">
                    {group.units.map((unit, index) => {
                      const attributes = formatAttributes(unit);
                      return (
                        <div key={`${unit.label}-${index}`} className="rounded-xl bg-slate-50 px-3 py-2">
                          <p>“{unit.span}”</p>
            ...
            <p className="mt-3 text-sm text-slate-500">No evidence units captured yet.</p>
          )}
        </Card>

        <div className="space-y-6">
          <Card className="border p-6">
            <h3 className="text-sm font-semibold text-slate-700">Unclear areas</h3>
            {unclearAreas.length ? (
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                {unclearAreas.map((prompt) => (
              ...
              <p className="mt-3 text-sm text-slate-500">No follow-ups suggested.</p>
            )}
          </Card>
          ...
          <Card className="border p-6">
            ...
            {questionsToExplore.length ? (
              ...
              <p className="mt-3 text-sm text-slate-500">No questions generated.</p>
            )}
          </Card>
        </div>
      </div>
...
export default EntryDetailV2Page;
