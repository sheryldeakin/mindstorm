...
const CyclesGraphPage = () => {
  ...
  const handlePatternSelect = useCallback((path: string[]) => {
    if (!path.length) return;
    const shouldClose = path.length >= 3 && path[0] !== path[path.length - 1];
    const finalPath = shouldClose ? [...path, path[0]] : path;
    const isCycle = finalPath.length > 1 && finalPath[0] === finalPath[finalPath.length - 1];
    setNeuralPaths([finalPath]);
    setNeuralActiveIndex(0);
    setNeuralSelection({
      start: finalPath[0],
      end: isCycle ? finalPath[0] : finalPath[finalPath.length - 1],
    });
  }, []);
